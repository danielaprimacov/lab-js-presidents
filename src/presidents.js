...
function countYearsInOffice(presidentsArr) {
  return presidentsArr.reduce((sumYears, currentPresident) => {
    if (currentPresident.leftOffice !== null) {
      return (
        sumYears + (currentPresident.leftOffice - currentPresident.tookOffice)
      );
    }
    return sumYears;
  }, 0);
...
function sortPresidentsByBirthYear(presidentsArr) {
  return presidentsArr.sort((president1, president2) => {
    if (president1.birthYear > president2.birthYear) return 1;
    if (president1.birthYear < president2.birthYear) return -1;
    return 0;
  });
}
...
function getAgeAtInauguration(presidentsArr) {
  return presidentsArr.map((president) => {
    const tookOfficeYear = president.tookOffice;
    const birthYear = president.birthYear;
    return {
      ...president,
      ageAtInauguration: tookOfficeYear - birthYear,
    };
  });
}
...
function countRepublicanPresidents(presidentsArr) {
  return presidentsArr.reduce((sumPresidents, currentPresident) => {
    if (currentPresident.party === "Republican") {
      return sumPresidents + 1;
    }
    return sumPresidents;
  }, 0);
}
...
function sortPresidentsByName(presidentsArr) {
  return presidentsArr.sort((president1, president2) => {
    if (president1.name > president2.name) return 1;
    if (president1.name < president2.name) return -1;
    return 0;
  });
}
